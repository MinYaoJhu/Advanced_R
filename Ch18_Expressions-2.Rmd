...
> Let's examine the AST structure of this assignment:

```{r}
ast(names(x) <- y)
```

> We want to handle the situation where both the first two elements are function calls. Moreover, if the first call is the same as <-, we only need to return the second call to determine which objects received new assigned values.

To accomplish this, we include the following code block within another else statement in the find_assign_call() function:
...
Let's conclude with the entire code, along with some tests for our new function:
